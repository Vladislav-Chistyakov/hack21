...
export function ChargingStationRow({
  mapPointId,
  state,
  charger,
}: ChargingStationRowProps) {
  const client = useQueryClient();
  const {
    isLoading,
    mutate: rent,
    isError,
  } = useRentChargingStation({
                  ...
                  [
                    {
                      ...
                      onPress() {
                        rent(
                          { chargerId: charger.id, mapPointId },
                          {
...
function getButtonText(state: RentStates) {
  ...
  if (state === "CANCELED") return "Отменено";
}

function getButtonTextColor(state: RentStates) {
  if (state === "FREE" || state === "SUCCESS" || state === "CANCELED")
    return "#FFFFFF";
  return "#111C35";
}

function getButtonBgColor(state: RentStates) {
  if (state === "UNAVAILABLE") return "#FFCF26";
  if (state === "IN_USE") return "#FFCF26";
  return "#111C35";
}
